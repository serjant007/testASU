...
function App() {
  ...
  const onEditPerson = (obj) => {
    ...
    setList(newList);
    axios
      .patch('http://localhost:3001/persons/' + obj.id, {
        id: obj.id,
  ...
  const onRemove = (item, id) => {
    if (window.confirm(` Вы действительно хотите удалить ${item.firstName} ${item.lastName}?`)) {
      axios
  ...
  return (
    <div className="App">
      <div className="container">
        <Header />
        {list ? (
          <Person showEdit={showEdit} items={list} onRemove={onRemove} editItem={editPersone} />
        ) : (
          <h4>Загрузка данных...</h4>
        )}
        <Button text="Добавить сотрудника" onCreate={showCreate} />
      </div>
      {showCreatePop && <Create list={list} onAdd={onAddPerson} closeCreatePop={closeCreate} />}
      {showEditPop && (
        <Edit
          list={list}
          onAdd={onEditPerson}
